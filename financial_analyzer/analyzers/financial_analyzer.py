...
class FinancialAnalyzer:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if self.api_key:
            ...
            self.client = None
            print("警告: 未提供Anthropic API密钥，AI分析功能将不可用")
    ...
    def _get_overall_rating(self, indicators: Dict) -> str:
        ...
        for dimension in ['profitability', 'solvency', 'operational', 'cashflow_quality']:
    ...
    def _identify_strengths(self, indicators: Dict) -> List[str]:
        ...
        if 'profitability' in indicators:
            ...
            if prof.get('gross_margin', 0) >= 40:
                strengths.append(f"毛利率高 ({prof['gross_margin']:.2f}%)，产品竞争力强")
            if prof.get('net_margin', 0) >= 15:
                strengths.append(f"净利率高 ({prof['net_margin']:.2f}%)，成本控制能力强")

        # 偿债能力优势
        if 'solvency' in indicators:
            solv = indicators['solvency']
            if solv.get('current_ratio', 0) >= 2:
                strengths.append(f"流动比率优秀 ({solv['current_ratio']:.2f})，短期偿债能力强")
            if solv.get('debt_to_asset', 100) <= 40:
                strengths.append(f"资产负债率低 ({solv['debt_to_asset']:.2f}%)，财务风险小")

        # 运营能力优势
        if 'operational' in indicators:
            oper = indicators['operational']
            if oper.get('asset_turnover', 0) >= 1.2:
                strengths.append(f"资产周转率高 ({oper['asset_turnover']:.2f})，资产使用效率优秀")
            if oper.get('cash_conversion_cycle', 999) <= 60:
                strengths.append(f"现金循环周期短 ({oper['cash_conversion_cycle']:.0f}天)，资金周转快")

        # 现金流优势
        if 'cashflow_quality' in indicators:
            cf = indicators['cashflow_quality']
            if cf.get('operating_cf_to_net_profit', 0) >= 100:
                strengths.append(f"经营现金流充沛 ({cf['operating_cf_to_net_profit']:.2f}%)，利润质量高")
    ...
    def _identify_weaknesses(self, indicators: Dict) -> List[str]:
        ...
        if 'profitability' in indicators:
            prof = indicators['profitability']
            if prof.get('roe', 0) < 5:
                weaknesses.append(f"净资产收益率较低 ({prof.get('roe', 0):.2f}%)，盈利能力不足")
            if prof.get('gross_margin', 0) < 20:
                weaknesses.append(f"毛利率偏低 ({prof.get('gross_margin', 0):.2f}%)，产品竞争力弱")
            if prof.get('net_margin', 0) < 5:
                weaknesses.append(f"净利率低 ({prof.get('net_margin', 0):.2f}%)，盈利空间小")

        # 偿债能力劣势
        if 'solvency' in indicators:
            solv = indicators['solvency']
            if solv.get('current_ratio', 0) < 1:
                weaknesses.append(f"流动比率不足1 ({solv.get('current_ratio', 0):.2f})，短期偿债压力大")
            if solv.get('debt_to_asset', 0) > 70:
                weaknesses.append(f"资产负债率高 ({solv.get('debt_to_asset', 0):.2f}%)，财务杠杆风险大")
        ...
        if 'operational' in indicators:
            ...
            if oper.get('asset_turnover', 0) < 0.5:
                weaknesses.append(f"资产周转率低 ({oper.get('asset_turnover', 0):.2f})，资产使用效率差")
            if oper.get('receivables_days', 0) > 90:
                weaknesses.append(f"应收账款周转天数长 ({oper.get('receivables_days', 0):.0f}天)，回款慢")
...
"""
    ...
    def generate_investment_recommendation(
        ...
        if rating == '强烈推荐':
            ...
            recommendation['action'] = '买入' if target_investor_type != '保守型' else '关注'
            recommendation['reasons'].append("公司财务状况良好，值得配置")
        elif rating == '中性':
            recommendation['action'] = '持有'
            recommendation['reasons'].append("公司财务状况一般，建议观望")
        elif rating == '不推荐':
            recommendation['action'] = '卖出' if target_investor_type == '激进型' else '减持'
        ...
        if analysis.get('risks'):
            recommendation['reasons'].extend([f"风险: {r}" for r in analysis['risks'][:2]])
